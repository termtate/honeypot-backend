...
class WebSocketManager:
    @inject
    def __init__(
        self, sockets_manager: SocketsManager, logger: Logger, settings: Settings
    ) -> None:
    ...
    async def _start_listening_sockets(self):
        buffered_stream = await pipe(
            self.sockets_manager.get_attack_stream(), buffer(self.interval.seconds)
        )
        ...
        async for attacks in observer:  # async for: 等待observer发送的attack，在收到attack时执行一轮循环，其他时间挂起，stream结束时退出循环
            ...
            await asyncio.gather(
                *[websocket.send_text(text) for websocket in self.websocket_connections]
            )
            self.logger.info(f"send attacks to websockets {self.websocket_connections}")
